...
def obter_import_linter():
    """Executa o Import Linter e extrai o resultado de cada contrato."""
    ...
    # O Import Linter lista um contrato por linha, no formato
    # "<nome do contrato> KEPT" ou "<nome do contrato> BROKEN" —
    # extrai todos, não só um contrato fixo, porque o projeto já
    # tem mais de um contrato configurado no .importlinter.
    contratos = {}
    for linha in saida.splitlines():
        match_contrato = re.match(r"^(.+?)\s+(KEPT|BROKEN)$", linha.strip())
        if match_contrato:
            contratos[match_contrato.group(1)] = match_contrato.group(2)
...
contratos_linter = import_linter["contratos"]

col1, col2, col3 = st.columns(3)

col1.metric(
    "Contratos mantidos",
    f"{sum(1 for s in contratos_linter.values() if s == 'KEPT')}/{len(contratos_linter)}"
    if contratos_linter else "N/A",
)
...
if contratos_linter:
    ...
    df_linter = pd.DataFrame(
        {
            "Contrato": list(contratos_linter.keys()),
            "Status": list(contratos_linter.values()),
        }
    )
    ...
    if all(status == "KEPT" for status in contratos_linter.values()):

        st.success(
            "Todos os contratos arquiteturais foram mantidos."
        )
